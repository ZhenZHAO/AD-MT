    ...
    for epoch_num in iterator:
        ...
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            if flag_start_self_learning: # strongest augs
                img_ulb_s, pseudo_outputs, pseudo_logits = cut_mix_3d(
                                img_ulb_s,
                                pseudo_outputs,
                                pseudo_logits
                )
                ...
                if alternate_indicator.get_alternate_state() == False:
                    ...
                    if np.random.random() < args.get("cutmix_prob", 1.0):
                        if mtx_bool_conflict is None:
                            ...
                            img_ulb_s, pseudo_outputs, pseudo_logits, mtx_bool_conflict = cut_mix_3d(
                                            img_ulb_w,
                                            pseudo_outputs,
                                            pseudo_logits,
                                            mtx_bool_conflict
        ...
        if epoch_num % args.get("test_interval_ep", 1) == 0 or iter_num >= max_iterations:
            ...
            if "pancreas" in args["root_path"].lower():
                dice_sample_stu = var_all_case_Pancrease(model, args["root_path"], num_classes=num_classes, patch_size=args["patch_size"], stride_xy=16, stride_z=16, flag_nms=True)
                dice_sample = var_all_case_Pancrease(ema_model, args["root_path"], num_classes=num_classes, patch_size=args["patch_size"], stride_xy=16, stride_z=16, flag_nms=True)
            else:
                dice_sample_stu = var_all_case_LA(model, args["root_path"], num_classes=num_classes, patch_size=args["patch_size"], stride_xy=18, stride_z=4)
                dice_sample = var_all_case_LA(ema_model, args["root_path"], num_classes=num_classes, patch_size=args["patch_size"], stride_xy=18, stride_z=4)
            ...
            if dice_sample > best_dice:
                ...
                save_mode_path = os.path.join(tmp_tea_snapshot_path, 'ep_{:0>3}_dice_{}.pth'.format(epoch_num, best_dice))
            ...
            # logs
            logging.info('iteration %d : dice_score: %f best_dice: %f' % (iter_num, dice_sample, best_dice))
            logging.info(" <<Test>> - Ep:{}  - Dice-S/T:{:.2f}/{:.2f}, Best-S:{:.2f}, Best-T:{:.2f}".format(epoch_num, 
                    dice_sample_stu*100, dice_sample*100, best_dice_stu*100, best_dice*100))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp', type=str,
                        default='LA/POST', help='experiment_name')
